...
- **Use `oci-bv-low-cost` (vpusPerGB: 0) for anything that isn't IOPS-hungry.** The default SC is `oci-bv` (10 VPU), which adds a "Block Volume - Performance Units" charge of ≈ £0.68/mo per 50 GB volume on top of ≈ £1.02 storage. Traefik and Redis have always run fine at 0 VPU. Migrated dmozdb + denokv to low-cost on 2026-09-07; ticketd's two volumes are the last on 10 VPU, left there because its sync replication has a 15 s quorum deadline (measure write latency before moving them).
- **Changing a PVC's storage class needs a data migration** — the field is immutable and OCI has no in-place VPU change via CSI. StatefulSet `volumeClaimTemplates` don't pin a `storageClassName` here, so they inherit the default; the migration works by **pre-creating the PVC under the exact name the template would generate** (`<template>-<sts>-<ordinal>`, e.g. `data-dmozdb-0`), which the StatefulSet then adopts instead of creating its own. Procedure: scale sts to 0 → mount the old PVC in a busybox pod and `tar cf - -C /vol .` to local disk → verify the tar → delete the PVC (reclaim is Delete, the OCI volume is destroyed) → create the replacement PVC with `storageClassName: oci-bv-low-cost` → mount it in a helper pod and `tar xf -` the data back → scale sts to 1. Pin helper pods to the node with `nodeSelector: kubernetes.io/hostname` since the class is `WaitForFirstConsumer`.
- **denokv's SQLite lives in the WAL.** `denokv.sqlite` is a 4 KB stub; the data is in `denokv.sqlite-wal` (~170 KB). Copy all three files (`.sqlite`, `-shm`, `-wal`) or a restore silently yields an empty database, and reading with `?immutable=1` reports `no such table: kv` because the WAL isn't replayed.
- **The dmozdb `web` deployment needs a restart after its backends restart.** Its startup probe latches; pods stay `0/1 Running` with no useful log line until `kubectl -n dmozdb rollout restart deploy web`.
